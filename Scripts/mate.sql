-- public.forwarded_latest_4_bh_mat_2 source  forwarded_latest_4_bh_mat_2
--DROP MATERIALIZED VIEW IF EXISTS forwarded_latest_4_bh_mat_2;


CREATE MATERIALIZED VIEW public.forwarded_latest_4_bh_mat_2
...
AS WITH latest_3 AS (
         ...
         SELECT a.rnn,
           ...
           FROM ( SELECT row_number() OVER (PARTITION BY gl.grievance_id ORDER BY gl.assigned_on DESC) AS rnn,
             ...
             JOIN latest_3 ON latest_3.grievance_id = a.grievance_id
          WHERE a.rnn = 1 AND latest_3.assigned_on < a.assigned_on
        )
 SELECT cmo_police_station_master.sub_district_id,
    ...
    grievance_master.received_at,2qqq
    latest_4.rnn,
...
-- public.atr_latest_11_mat source

CREATE MATERIALIZED VIEW public.atr_latest_11_mat
TABLESPACE pg_default
AS WITH latest_7 AS (
         SELECT a.grievance_id,
            a.assigned_on
           FROM ( SELECT row_number() OVER (PARTITION BY gl.grievance_id ORDER BY gl.assigned_on DESC) AS rnn,
                    ...
                    gl.assigned_on
                   FROM grievance_lifecycle gl
                  WHERE gl.grievance_status = ANY (ARRAY[7, 12])) a
          WHERE a.rnn = 1
        ), latest_11 AS (
         SELECT a.rnn,
            a.lifecycle_id,
           ...
           FROM ( SELECT row_number() OVER (PARTITION BY gl.grievance_id ORDER BY gl.assigned_on DESC) AS rnn,
                    ...
                    gl.migration_id_ac_tkn
                   FROM grievance_lifecycle gl
                  WHERE gl.grievance_status = 11 AND gl.assigned_by_office_cat in (2,3)) a
             JOIN latest_7 ON latest_7.grievance_id = a.grievance_id
          WHERE a.rnn = 1 AND latest_7.assigned_on < a.assigned_on
        )
 SELECT cmo_police_station_master.sub_district_id,
    grievance_master.district_id,
    grievance_master.block_id,
