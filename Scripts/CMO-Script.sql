/* TAB_ID: 2B6 | TBL: All >> Role: 5 Ofc: 113 | G_Codes: ('GM011', 'GM013', 'GM006') */
                with lastupdates AS (
                    select
                ...
                master_district_block_grv_data AS (
                    select distinct
                        md.grievance_id,
                        case
                            when (select grievance_lifecycle.assigned_to_office_id from grievance_lifecycle where grievance_lifecycle.grievance_id = md.grievance_id and grievance_lifecycle.grievance_status = 3 order by grievance_lifecycle.assigned_on desc limit 1) = 113 then 0
                            else 1
                    ...
                    left join lastupdates lu on lu.rn = 1 and lu.grievance_id = md.grievance_id and lu.assigned_to_office_id = 113
                    ...
                    where md.status in (6,11,13) and md.assigned_to_office_id = 113
                        and md.assigned_to_position = 1060
...
--- testing otp ----
select * from migration_testing.public.user_otp uo where uo.u_phone = '9874263537' order by created_on desc;